...
def boavinda(message1):
    ...
    if ide in liste:
        ...
        bot.send_message(ide, '<b>' '✅ Adquira acesso com meu pai @Anonylions ✅' '</b>', parse_mode='HTML')
...
def zn(nome):
            ...
            if id1 in ltnome:
                try:
                    ...
                    fl = msg.split('/cpf')
                    ip = re.sub('[^0-9]', '', msg)
                    url = requests.get('https://cadsusnonny.000webhostapp.com/cpfsusDK.php?token=9DatOhxI2jlu2k1&cpf=' + ip)
                    req = url.json
                	...
                	bot.reply_to(nome, '<b>' 'ERRADO OU CONSULTA JA FEITA' '</b>', parse_mode='HTML')
            else:
